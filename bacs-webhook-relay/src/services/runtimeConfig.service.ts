...
class RuntimeConfigService {
  async load(): Promise<BacsWebhookRelayConfig> {
    ...
    try {
      const raw = await ssmConfig.getParameter(env.BACS_WEBHOOK_RELAY_BATCH_SIZE_PARAM);
      ...
      log.warn(METHOD.LOAD, LOG_MESSAGES.SSM_PARAMETER_LOAD_FAILED, {
        param: env.BACS_WEBHOOK_RELAY_BATCH_SIZE_PARAM,
