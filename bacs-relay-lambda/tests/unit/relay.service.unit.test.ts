...
describe('RelayService.execute', () => {
  it('returns zero counts when there are no pending rows', async () => {
    ...
    expect(mockedSqs.sendToBacsRelayQueue).not.toHaveBeenCalled();
  });
  ...
  it('enqueues each pending row and marks it via repository.markEnqueued', async () => {
    ...
    mockedSqs.sendToBacsRelayQueue.mockResolvedValue({ MessageId: 'sqs-msg-id' } as Awaited<ReturnType<typeof sqsConfig.sendToBacsRelayQueue>>);
    ...
    expect(mockedSqs.sendToBacsRelayQueue).toHaveBeenCalledTimes(2);
    ...
    expect(mockedSqs.sendToBacsRelayDeadLetterQueue).not.toHaveBeenCalled();
  });
  ...
  it('uses webhook_id as the SQS MessageDeduplicationId', async () => {
    ...
    mockedSqs.sendToBacsRelayQueue.mockResolvedValue({ MessageId: 'm' } as Awaited<ReturnType<typeof sqsConfig.sendToBacsRelayQueue>>);

    await relayService.execute();

    expect(mockedSqs.sendToBacsRelayQueue).toHaveBeenCalledWith(
  ...
  it('routes poison messages to DLQ and marks them dead_letter', async () => {
    ...
    mockedSqs.sendToBacsRelayDeadLetterQueue.mockResolvedValue({ MessageId: 'dlq-1' } as Awaited<ReturnType<typeof sqsConfig.sendToBacsRelayDeadLetterQueue>>);
    ...
    expect(mockedSqs.sendToBacsRelayDeadLetterQueue).toHaveBeenCalledTimes(1);
  ...
  it('counts transient SQS failures as "failed" and leaves the row for next invocation', async () => {
    ...
    mockedSqs.sendToBacsRelayQueue.mockRejectedValue(new Error('boom'));
